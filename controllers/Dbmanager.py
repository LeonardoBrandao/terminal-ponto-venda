...
class Dbmanager:
    ...
    @staticmethod
    def setProduct(prod):
        f = open(os.path.abspath('produtos.txt'), 'a')
        f.write(prod)
        f.close()

    @staticmethod
    def getProduct(prod_id):
        produtos_txt = open(os.path.abspath('produtos.txt'), 'r').readlines()
    ...
    def setOrder(order):
        ...
        f = open(os.path.abspath('pedidos.txt'), 'a')
        f.write(pedido)
        f.close()

    @staticmethod
    def getOrder(order_id):
        pedidos_txt = open(os.path.abspath('pedidos.txt'), 'r').readlines()
    ...
    def alterStateOrder(id, state):
        pedidos_txt = open(os.path.abspath('pedidos.txt'), 'r').readlines()
        f = open(os.path.abspath('pedidos.txt'), 'w')
        for pedido_line in pedidos_txt:
            pedido = pedido_line.strip(':\n').split(',')
            if pedido[0] != id:
                f.write(pedido_line)
    ...
    def getCustomer(id = False, identif = False):
        clientes_txt = open(os.path.abspath('clientes.txt'), 'r').readlines()
    ...
    def saveCustomer(infos):
        f = open(os.path.abspath('clientes.txt'), 'a')
        infos_save = ''
    ...
    def relCustomerOrder(cid, oid):
        f = open(os.path.abspath('relClientePedido.txt'), 'a')
        f.write(cid + ',' + oid + '\n')
        f.close()
    ...
    def getCustomerOrder(cli = False, ord = False):
        if cli:
            txt = open(os.path.abspath('relClientePedido.txt'), 'r').readlines()
            for line in txt:
                content = line.strip('\n').split(',')
    ...
    def getAllCustomerOrders():
        co_txt = open(os.path.abspath('relClientePedido.txt'), 'r').readlines()
        co = []
    ...
    def setNF(tipo, date, total, order_id):
        ...
        f = open(os.path.abspath('nfs.txt'), 'a')
    ...
    @staticmethod
    def getNF(order_id):
        f = open(os.path.abspath('nfs.txt'), 'r').readlines()
        for line in f:
            infos = line.strip('\n').split(':')
